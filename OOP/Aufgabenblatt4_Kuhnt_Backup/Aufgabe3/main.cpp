...
//Ich weiß nicht so recht wie ich das mit den Parametrisierten Datentypen machen soll...
//Die Zeit wird auch ein bisschen knapp, deswegen gebe ich es jetzt trotzdem ab, obwohl ich weiß das es nicht fertig ist...
//Ich werde mich aber sofort weiter dran setzen!
...
/*bool find(double* array, int size, double key){
...
int main()
{
    try{
		...
		std::cout << "Ist die Zahl 15.5 in dem Array vorhanden?" << ArrayList::find(arrayList1.getArray(), arrayList1.getSize(), 15.5) << std::endl;
		
		std::cout << "Ist die Zahl 1.5 in dem Array vorhanden?" << ArrayList::find(arrayList1.getArray(), arrayList1.getSize(), 1.5) << std::endl;
